...
echo -e "${BLUE}Setting up Caddy for automatic HTTPS...${NC}"

PUBLIC_IP=$(curl -s https://ifconfig.me || echo "your-vps-ip")
DEFAULT_DOMAIN="$PUBLIC_IP.sslip.io"

echo -n -e "Enter your domain (press Enter for $DEFAULT_DOMAIN): "
...
DOMAIN_NAME=$(grep -oE '^[^ {]+' /etc/caddy/Caddyfile | head -n 1)

echo -e "Server address: ${GREEN}https://$DOMAIN_NAME${NC}"
